...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item active">Ledger Edit</li>
</ol>
</div>
</div>
</div><!-- /.container-fluid -->
        ...
        <div class="col-md-12">
          ...
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Please Edit <small>Ledger Data</small></h3>
            </div>
            <!-- /.card-header -->
            <?php 
           
            $id = $_REQUEST['id'];
            $edit_query = "SELECT * FROM snit_general_ledger_list WHERE id = $id";
            $result = $db->query($edit_query);
            $row = mysqli_fetch_array($result);
            ?>
            <!-- form start -->
            <form id="quickForm" action="general_ledger_edit_action.php?id=<?php echo $id ?>" method="POST" enctype="multipart/form-data">
              <div class="card-body">
                <div class="form-group">
                  <label for="exampleInputEmail1">Ledger Name</label>
                  <input type="text" name="firstname" class="form-control" id="exampleInputEmail1" placeholder="Enter First Name" value="<?php echo $row['ledger_name']; ?>">
                </div>
                <div class="form-group">
                  <label for="exampleInputPassword1">Ledger Type</label><br>
                  <input type="radio"  name="ledger_type" <?php if($row['ledger_type'] == 1){ ?>  checked="checked"  <?php } ?> value='1'>
                  <label for="male">Income Statement</label> 
                  <input type="radio"  name="ledger_type" <?php if($row['ledger_type'] == 0){ ?>  checked="checked"  <?php } ?> value='0'>
