      ...
      <table class="table table-striped projects">
        ...
        <tbody>
          <?php
            ...
            $total_pages_sql = "SELECT COUNT(id) AS total_rows FROM snit_user_list WHERE 1";
            $res = $database->conn->query($total_pages_sql);
            $row = mysqli_fetch_array($res);
            $total_rows = $row['total_rows'];
            $total_pages = ceil($total_rows / $no_of_records_per_page);

            $user_query = "SELECT * FROM snit_user_list WHERE 1 ORDER BY id ASC LIMIT $offset, $no_of_records_per_page";
